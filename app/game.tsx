...
export default function Game() {
  ...
  const showTower = () => {
    ...
    if (gameState.tower.nextBlockPull <= 0 && gameState.dishonesty <= 0) {
      //we can skip showing you the tower if you don't need to pull blocks
      return;
    }
  ...
  const checkGameOverAndRoute = () => {
    if (gameState.tower.collapsed) {
      updateGameState({
        ...gameState,
        gameOver: true,
        gameOverReason: "towerCollapsed",
      });
  ...
  return (
    <>
      {gameState.turnCount === 0 && <IntroText />}
      {view === "game" && (
        <Swipeable
